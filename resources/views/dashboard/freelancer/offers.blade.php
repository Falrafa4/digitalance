        ...
        @if($offers->isEmpty())
            <div class="text-center py-16 px-5 bg-white border-2 border-dashed border-slate-200 rounded-[20px]">
                ...
                <a href="{{ route('freelancer.orders.index') }}"
                    class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#0f766e] text-white rounded-xl font-bold text-[13px] hover:bg-[#0d6b63] transition-all shadow-teal-sm">
        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                @foreach($offers as $offer)
                    <div
                        class="bg-white border border-slate-200 rounded-[20px] p-5 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                        <div class="flex items-start justify-between mb-4 pb-4 border-b border-slate-100">
                            <div>
                                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1">Order
                                    #{{ $offer->order_id }}</span>
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            <div
                                class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 overflow-hidden">
                        ...
                        <div class="bg-slate-50 rounded-xl p-4 mb-4">
                            ...
                            <p class="text-[18px] font-extrabold text-[#0f766e]">Rp
                                {{ number_format($offer->offered_price, 0, ',', '.') }}</p>
                        </div>

                        <div class="bg-white border border-slate-100 rounded-xl p-4 mb-4 text-[13px] text-slate-600 truncate"
                            title="{{ $offer->message }}">
                        ...
                        @if($offer->status === 'Sent')
                            ...
                            <div class="text-center text-xs font-bold text-amber-600 bg-amber-50 rounded-lg p-2">Menunggu konfirmasi
                                klien</div>
                        @endif
                    </div>
                @endforeach
