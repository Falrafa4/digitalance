...
<div class="animate-fadeUp flex-1 px-8 py-7 overflow-y-auto">
    <div class="flex items-end justify-between mb-8 gap-4 flex-wrap">
        <div>
            <h1 class="font-display text-[2.1rem] font-extrabold text-slate-900 leading-tight">Portofolio Saya</h1>
                ...
                <div class="bg-white border border-slate-200 rounded-[20px] overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
                    <div class="h-[180px] w-full relative bg-slate-100">
                        ...
                        <div class="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                            ...
                            <button onclick="window.openPortDeleteModal({{ $port->id }})" class="w-10 h-10 rounded-full bg-white text-slate-700 flex items-center justify-center hover:bg-red-50 hover:text-red-600 transition-colors">
...
<script>
    ...
    window.openPortDeleteModal = function(id) {
        ...
        const btn = document.getElementById('btn-confirm-delete-port');
    ...
    window.closePortDeleteModal = function() {
        const overlay = document.getElementById('modal-delete-port-overlay');
