    ...
    <div class="mb-8 animate-fadeUp">
        ...
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
                ...
                <h1 class="font-display text-[2rem] font-extrabold text-slate-900 leading-tight">{{ $loker->title }}</h1>
            </div>
        </div>
    </div>
        ...
        <div class="bg-indigo-50 border border-indigo-200 rounded-[18px] p-5 mb-5 text-center animate-fadeUp">
            <p class="text-[13px] font-bold text-indigo-700">
                ...
                <span class="text-[12px] font-semibold text-indigo-500 ml-1">
                    Status: {{ $loker->myApplication->status === 'Approved' ? 'Diterima' : ($loker->myApplication->status === 'Rejected' ? 'Ditolak' : 'Menunggu persetujuan') }}
                </span>
            </p>
        </div>
    @else
