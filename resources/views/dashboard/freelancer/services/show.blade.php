...
@section('content')
    <div class="animate-fadeUp max-w-4xl mx-auto px-4 py-8">
        <div class="mb-6 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div class="flex items-center gap-2">
                @if($service->status === 'Draft')
                    <form id="form-submit-service" action="{{ route('freelancer.services.submit', $service->id) }}"
                        method="POST">
                        @csrf
                        <button type="button" onclick="window.confirmSubmitService()"
                            class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-orange-500 text-white font-semibold hover:bg-orange-600 transition-colors shadow-orange-sm">
                            <i class="ri-send-plane-fill"></i> Ajukan ke Admin
                        </button>
                    </form>
                @endif
        ...
        @if($service->reject_reason)
            <div
                class="mb-6 p-5 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-4 shadow-sm animate-pulse-slow">
                ...
                <div>
                    ...
                    <p class="text-amber-600 text-[11px] mt-2 font-bold uppercase tracking-wider italic">* Silakan edit dan
                        ajukan kembali layanan ini agar dapat dipublikasikan.</p>
                </div>
            </div>
        @endif
