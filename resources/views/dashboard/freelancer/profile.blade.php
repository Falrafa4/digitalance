...
@section('content')
    <div class="content-scroll flex-1 px-4 sm:px-8 py-7 overflow-y-auto">
        ...
        <div class="mb-8 animate-fadeUp">
            ...
            <p class="text-slate-500 text-[0.95rem] mt-1">Kelola informasi dasar dan keamanan akun kamu.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1">
                <div
                    ...
                    <span
                        class="inline-flex items-center gap-1.5 mt-2 px-3 py-1 bg-[#fff7ed] text-[#f97316] text-[11px] font-bold rounded-full">
                        <i class="ri-vip-crown-line"></i> Freelancer
                    </span>
                    @if($user->skomda_student)
                        <p class="text-xs text-slate-500 mt-3 font-semibold">Terkoneksi dengan akun Skomda Student (NIS: {{ $user->skomda_student->nis }})</p>
                    @endif
                </div>
            </div>
            ...
            <div class="lg:col-span-2 flex flex-col gap-5">
                ...
                <div class="bg-white rounded-[18px] border border-slate-200 animate-fadeUp-1">
                    <div class="px-7 py-5 border-b border-slate-100">
                        ...
                        <p class="text-[12px] text-slate-400 mt-0.5">Perbarui data kontak (bio dan nomor telepon).</p>
                    </div>
                    <div class="px-7 py-6">
                        @if(session('success'))
                            <div
                        ...
                        <form action="{{ route('freelancer.profile.update') }}" method="POST">
                            ...
                            <div class="flex flex-col gap-1.5 mb-4">
                                ...
                                <textarea name="bio" rows="3" class="py-[10px] px-[13px] bg-slate-50 border-[1.5px] border-slate-200 rounded-[11px] text-[13.5px] outline-none transition-all duration-200 focus:border-[#0f766e] focus:bg-white focus:shadow-[0_0_0_3px_rgba(15,118,110,0.08)] resize-none @error('bio') border-red-400 @enderror">{{ old('bio', $user->bio) }}</textarea>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    <input type="text" value="{{ $user->name }}" disabled class="py-[10px] px-[13px] bg-slate-100 text-slate-500 border-[1.5px] border-slate-200 rounded-[11px] text-[13.5px] outline-none cursor-not-allowed" />
                                ...
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    <input type="email" value="{{ $user->email }}" disabled class="py-[10px] px-[13px] bg-slate-100 text-slate-500 border-[1.5px] border-slate-200 rounded-[11px] text-[13.5px] outline-none cursor-not-allowed" />
                                </div>
                            </div>
                ...
                <div class="bg-white rounded-[18px] border border-red-100 animate-fadeUp-3">
                    ...
                    <div class="px-7 py-6 flex items-center justify-between flex-wrap gap-4">
                        <div>
                            <p class="font-semibold text-[14px] text-slate-900">Hapus Akun</p>
                            <p class="text-[12px] text-slate-400 mt-0.5">Semua data akan dihapus permanen.</p>
                        </div>
                        <form action="{{ route('freelancer.delete') }}" method="POST"
                            onsubmit="event.preventDefault(); customConfirm('Yakin ingin menghapus akun? Aksi ini permanen.').then(res => { if(res) this.submit(); });">
                            @csrf
                            <input type="hidden" name="password" value="will_be_prompted_in_full_version" id="del-pass" />
                            <button type="submit"
