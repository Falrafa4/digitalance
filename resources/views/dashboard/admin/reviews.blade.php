...
@section('title', 'Manajemen Ulasan | Digitalance')
@section('styles')
...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 animate-fadeUp">
        <div>
            ...
            <p class="text-slate-500 text-[0.95rem] mt-1">Kelola ulasan dan penilaian yang diberikan oleh pengguna platform.
            </p>
        </div>
        <div class="flex items-center gap-3">
            <div class="bg-white px-5 py-3 rounded-2xl border border-slate-100 flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-xl bg-amber-50 text-amber-500 flex items-center justify-center text-lg shadow-sm">
    ...
    <div class="flex items-center justify-between gap-4 mb-8 flex-wrap animate-fadeUp-2">
        ...
        <form action="{{ route('admin.reviews.index') }}" method="GET" class="relative">
            ...
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari klien atau layanan..."
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fadeUp-3">
            @foreach($reviews as $review)
                <div class="bg-white border border-slate-200 rounded-[24px] p-6 hover:shadow-lg transition-all duration-300">
                    <div class="flex items-start justify-between mb-5">
                        <div class="flex items-center gap-3.5">
                            ...
                            <div>
                                ...
                                <p class="text-slate-400 text-[11px] font-bold uppercase tracking-wider mt-0.5">Pesanan
                                    #{{ $review->order_id }}</p>
                            </div>
                        </div>
                    ...
                    <div class="mb-5">
                        <p class="text-slate-500 text-[11px] font-bold uppercase tracking-widest mb-1.5">Layanan</p>
                        <p class="text-slate-800 font-bold text-[13.5px] truncate">
                            {{ $review->order->service->title ?? 'Tidak tersedia' }}</p>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-2 border-t border-slate-50">
                        ...
                        <button onclick="window.openDeleteReview({{ $review->id }})"
                            class="text-[11px] font-bold text-red-500 hover:text-red-700 uppercase tracking-wider">Hapus
                            Ulasan</button>
                    </div>
                </div>
            @endforeach
    ...
    @else
        <div class="py-24 text-center animate-fadeUp-3">
            <div
                class="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-5 text-[2.5rem] text-slate-300">
    ...
    <div class="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center opacity-0 pointer-events-none transition-all duration-300"
        ...
        <div
            ...
            <div class="px-8 pt-8 pb-6 text-center">
                ...
                <p class="text-[13.5px] text-slate-500 leading-relaxed">Ulasan ini akan dihapus permanen dan tidak dapat
                    dikembalikan.</p>
            </div>
            <div class="flex gap-3 px-8 pb-8">
                <button onclick="window.closeDeleteReview()"
                ...
                <button id="btn-confirm-delete-review"
