            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-100">
                        ...
                        <th class="px-6 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Client &
                            Freelancer</th>
                ...
                <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    ...
                                    <span
                                        class="text-[11px] text-slate-400 font-bold uppercase mt-0.5 truncate max-w-[150px]">{{ $order->service->title ?? 'N/A' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col gap-1">
                                    <div class="flex items-center gap-2">
                                        <div
                                            ...
                                            C</div>
                                        <span
                                            class="text-[12px] font-bold text-slate-700">{{ $order->client->name ?? 'N/A' }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <div
                                            ...
                                            F</div>
                                        <span
                                            class="text-[12px] font-medium text-slate-500">{{ $order->service->freelancer->skomda_student->name ?? 'N/A' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                    ...
                    @empty
                        <tr>
                            <td colspan="6">
                                <x-ui.empty-state icon="ri-file-list-3-line" title="No Orders Found"
...
@section('scripts')
    ...
    <script>
            ...
            <p class="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-8">Order ID: #ORD-${o.id}</p>
                ...
                <div class="space-y-6 mb-10">
                    <div class="field-group">
                        <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">New
                            Status</label>
                        <div class="relative">
                            <select name="status"
                                class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 font-bold focus:border-[#0f766e] focus:bg-white outline-none transition-all appearance-none">
                                ${['Pending', 'Negotiated', 'Paid', 'In Progress', 'Revision', 'Completed', 'Cancelled'].map(s => `
                                <option value="${s}" ${o.status === s ? 'selected' : ''}>${s}</option>
                                `).join('')}
                            </select>
                            <i
                                class="ri-arrow-down-s-line absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"></i>
                        </div>
                    </div>
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="window.closeOrderModal()"
                        class="flex-1 py-4 bg-slate-100 text-slate-500 font-bold rounded-2xl text-[13px] hover:bg-slate-200 transition-all">Cancel</button>
                    <button type="submit"
                        class="flex-1 py-4 bg-[#0f766e] text-white font-bold rounded-2xl text-[13px] hover:bg-[#0a5e58] transition-all shadow-lg shadow-teal-sm">Update
                        Status</button>
                </div>
            </form>
        </div>
        ...
        // Delegate click handlers for buttons to avoid inline JS with Blade expressions
        ...
        <div class="relative">
            <div class="h-28 bg-gradient-to-r from-teal-600 to-emerald-600 flex items-center px-8">
                ...
                <button onclick="window.closeOrderModal()"
                    class="w-10 h-10 bg-white/20 text-white rounded-full flex items-center justify-center hover:bg-white/30 transition">
                    <i class="ri-close-line text-xl"></i>
                </button>
            </div>
            ...
            <div class="px-8 pb-8 -mt-8 relative z-10">
                <div class="bg-white rounded-2xl p-6 shadow-xl border border-slate-50 mb-6">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 class="text-[13px] font-bold text-slate-400 uppercase tracking-widest mb-1">Service Purchased
                            </h3>
                            <p class="text-[1.1rem] font-black text-slate-900 line-clamp-2 leading-tight">${o.service?.title ||
                'N/A'}</p>
                        </div>
                        <span
                            class="px-3 py-1 bg-teal-50 text-[#0f766e] text-[10px] font-black rounded-lg uppercase tracking-wider border border-teal-100">${o.status}</span>
                    </div>
                    <div class="flex items-center justify-between pt-4 border-t border-slate-50">
                        <span class="text-[13px] font-bold text-slate-500">Order Amount</span>
                        <span class="text-[1.3rem] font-black text-[#0f766e]">Rp${(o.agreed_price ||
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                        <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Purchased
                            By</span>
                        <div class="flex items-center gap-3">
                            <div
                                class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-[11px]">
                                C</div>
                            <span class="text-[13px] font-bold text-slate-700 truncate">${o.client?.name || 'Client'}</span>
                        </div>
                    </div>
                    <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                        <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Fulfilled
                            By</span>
                        <div class="flex items-center gap-3">
                            <div
                                class="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center font-black text-[11px]">
                                F</div>
                            <span
                                class="text-[13px] font-bold text-slate-700 truncate">${o.service?.freelancer?.skomda_student?.name
                || 'Freelancer'}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex gap-3">
                    <button onclick="window.openOrderModal(${o.id})"
                        class="flex-1 py-4 bg-slate-900 text-white font-bold rounded-2xl text-[13px] hover:bg-slate-800 transition-all flex items-center justify-center gap-2 shadow-lg shadow-slate-200">
