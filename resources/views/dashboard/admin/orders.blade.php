                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex flex-col gap-2">
                                                ...
                                                @endphp
                         <span
                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest {{ $payoutDone ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-amber-50 text-amber-700 border border-amber-100' }}">
...
@section('scripts')
    ...
    <script>
        ...
        window.openTransferOrder = function (id) {
            ...
            box.innerHTML = `
                <div class="p-8">
                    ...
                    <p class="text-slate-400 text-[11px] font-bold uppercase tracking-widest mb-6">Order ID: #ORD-${o.id}</p>
                    <div class="mb-6 p-4 ${payoutDone ? 'bg-emerald-50 border-emerald-100' : 'bg-amber-50 border-amber-100'} rounded-2xl border">
                        ...
                        ${payoutDone ? `<div class="mt-3 text-[11px] text-emerald-700 font-bold">Tanggal transfer: ${payoutDate}</div>` : ''}
                    </div>
                    <form action="/admin/orders/${o.id}/transfer" method="POST">
        ...
        document.addEventListener('click', function (e) {
            ...
            var transferBtn = e.target.closest && e.target.closest('.btn-open-transfer-order');
            if (transferBtn) {
                var id4 = transferBtn.getAttribute('data-order-id');
                if (id4) window.openTransferOrder(id4);
                return;
            }

            var transferDetailBtn = e.target.closest && e.target.closest('.btn-open-transfer-detail');
            if (transferDetailBtn) {
                var id5 = transferDetailBtn.getAttribute('data-order-id');
                if (id5) window.openTransferOrder(id5);
                return;
            }
