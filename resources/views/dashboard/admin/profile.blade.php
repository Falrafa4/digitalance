...
<div class="content-scroll flex-1 px-4 sm:px-8 py-7 overflow-y-auto">
    ...
    <div class="mb-8 animate-fadeUp">
        ...
        <p class="text-slate-500 text-[0.95rem] mt-1">Kelola informasi dan keamanan akun administrator kamu.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-3xl border border-slate-200 p-6 flex flex-col items-center text-center animate-fadeUp">
                <div class="relative mb-4">
                    <img id="avatar-preview"
                        src="{{ Auth::user()->profile_photo ? asset('storage/'.Auth::user()->profile_photo) : 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name).'&background=0f766e&color=fff&size=128' }}"
                        class="w-24 h-24 rounded-3xl object-cover border-4 border-white shadow-teal-md" />
                    <label for="photo-input" class="absolute -bottom-2 -right-2 w-8 h-8 bg-[#0f766e] text-white rounded-xl flex items-center justify-center cursor-pointer shadow-teal-sm hover:bg-[#0a5e58] transition-all duration-150">
                        <i class="ri-camera-line text-[14px]"></i>
                    </label>
                    <input type="file" id="photo-input" accept="image/*" class="hidden" onchange="previewAvatar(this)" />
                </div>
                ...
                <span class="inline-flex items-center gap-1.5 mt-2 px-3 py-1 bg-[#f0fdf9] text-[#0f766e] text-[11px] font-bold rounded-full">
                    <i class="ri-shield-user-line"></i> Administrator
                </span>
                <div class="w-full pt-5 mt-5 border-t border-slate-100 grid grid-cols-2 gap-3 text-center">
                    <div>
                        <p class="font-display font-extrabold text-[1.4rem] text-slate-900">{{ $totalAdmins ?? '—' }}</p>
                        <p class="text-[11px] text-slate-400">Total Admin</p>
                    </div>
                    <div>
                        <p class="font-display font-extrabold text-[1.4rem] text-slate-900">{{ $totalUsers ?? '—' }}</p>
                        <p class="text-[11px] text-slate-400">Total User</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 flex flex-col gap-5">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 animate-fadeUp-1">
                <div class="px-7 py-5 border-b border-slate-100">
                    ...
                    <p class="text-[12px] text-slate-400 mt-0.5">Perbarui data diri administrator.</p>
                </div>
                <div class="px-7 py-6">
                    @if(session('success'))
                    ...
                    <form action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data" id="profile-form">
                        @csrf
                        @method('PATCH')
                        <input type="file" name="profile_photo" id="photo-hidden" class="hidden" />
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <input type="text" name="phone" value="{{ old('phone', Auth::user()->phone) }}" placeholder="+62 812-xxxx-xxxx"
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <input type="text" name="location" value="{{ old('location', Auth::user()->location) }}" placeholder="Surabaya, Jawa Timur"
                                    class="py-[10px] px-[13px] bg-slate-50 border-[1.5px] border-slate-200 rounded-[11px] text-[13.5px] outline-none transition-all duration-200 focus:border-[#0f766e] focus:bg-white focus:shadow-[0_0_0_3px_rgba(15,118,110,0.08)]" />
                            </div>
                        </div>
                        <div class="flex flex-col gap-1.5 mb-6">
                            <label class="text-[11px] font-bold text-slate-500 uppercase tracking-[.1em]">Bio</label>
                            <textarea name="bio" rows="3" placeholder="Deskripsi singkat…"
                                class="py-[10px] px-[13px] bg-slate-50 border-[1.5px] border-slate-200 rounded-[11px] text-[13.5px] outline-none transition-all duration-200 resize-y focus:border-[#0f766e] focus:bg-white focus:shadow-[0_0_0_3px_rgba(15,118,110,0.08)]">{{ old('bio', Auth::user()->bio) }}</textarea>
                        </div>
                    ...
                    <form action="{{ route('admin.profile.password') }}" method="POST">
                        ...
                        <div class="flex flex-col gap-1.5 mb-4">
                            ...
                            <div class="relative">
                                ...
                                <button type="button" onclick="togglePass('cur-pass', this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 bg-transparent border-none cursor-pointer hover:text-slate-600 transition-all duration-150"><i class="ri-eye-line text-[16px]"></i></button>
                            </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <button type="button" onclick="togglePass('new-pass', this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 bg-transparent border-none cursor-pointer hover:text-slate-600 transition-all duration-150"><i class="ri-eye-line text-[16px]"></i></button>
                                </div>
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="relative">
                                    ...
                                    <button type="button" onclick="togglePass('conf-pass', this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 bg-transparent border-none cursor-pointer hover:text-slate-600 transition-all duration-150"><i class="ri-eye-line text-[16px]"></i></button>
                                </div>
                            </div>
                        </div>
                        <div class="flex justify-end">
