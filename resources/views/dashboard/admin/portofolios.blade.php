...
@section('title', 'Manajemen Portofolio | Digitalance')
@section('styles')
...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 animate-fadeUp">
        ...
        <div class="flex items-center gap-3">
            <div class="bg-white px-5 py-3 rounded-2xl border border-slate-100 flex items-center gap-3 shadow-sm">
                <div
                    class="w-10 h-10 rounded-xl bg-teal-50 text-teal-600 flex items-center justify-center text-lg shadow-sm">
                    <i class="ri-gallery-line"></i>
                </div>
                <div>
                    <div class="text-[1.2rem] font-black text-slate-900 leading-none">{{ $portofolios->total() }}</div>
                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Total Karya</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between gap-4 mb-10 flex-wrap animate-fadeUp-2">
        <form action="{{ route('admin.portofolios.index') }}" method="GET" class="flex gap-2 flex-wrap items-center">
            <div class="relative">
                ...
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari judul atau freelancer..."
            ...
            @if(request('q'))
                <a href="{{ route('admin.portofolios.index') }}"
                    class="px-4 py-2 text-slate-400 hover:text-red-500 text-xs font-black uppercase tracking-widest transition-all">Hapus
                    Pencarian</a>
            @endif
        </form>
    </div>
    ...
    @if($portofolios->count() > 0)
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-7 animate-fadeUp-3">
            @foreach($portofolios as $p)
                <div class="port-card bg-white rounded-[24px] border border-slate-100 overflow-hidden shadow-sm hover:shadow-xl hover:border-[#0f766e]/30 group cursor-pointer"
                    onclick="window.openPortDetail({{ $p->id }})">
                    <div class="relative aspect-[4/3] overflow-hidden bg-slate-100">
                        ...
                        <div
                            class="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-5">
                    ...
                    <div class="p-5">
                        <div class="flex items-center gap-2 mb-2">
                            <span
                                class="text-[10px] font-black text-[#0f766e] bg-[#f0fdfa] px-2 py-0.5 rounded-lg uppercase tracking-wider">#PORT-{{ $p->id }}</span>
                            <span
                                class="text-[10px] font-bold text-slate-400 truncate flex-1">{{ $p->service->title ?? 'Umum' }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-slate-50">
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $p->created_at->format('d M Y') }}</span>
                            </div>
                            <div
                                class="w-7 h-7 rounded-lg bg-slate-50 text-slate-400 flex items-center justify-center group-hover:bg-[#0f766e] group-hover:text-white transition-all">
    ...
    @else
        <div class="py-32 text-center animate-fadeUp-3">
            <div
                class="w-24 h-24 bg-slate-50 rounded-[32px] flex items-center justify-center mx-auto mb-6 text-[2.8rem] text-slate-200">
                <i class="ri-gallery-line"></i>
            </div>
            <h3 class="text-[1.4rem] font-black text-slate-900">No Portfolios Yet</h3>
            <p class="text-slate-500 max-w-[350px] mx-auto mt-2 font-medium">Belum ada karya yang diunggah oleh para freelancer
                kami.</p>
        </div>
    @endif
@endsection
...
@section('modals')
    <div class="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center opacity-0 pointer-events-none transition-all duration-300"
        id="modal-port-overlay">
        <div class="bg-white rounded-[32px] w-full max-w-[540px] shadow-2xl overflow-hidden transform scale-95 transition-all duration-300"
            id="modal-port-box">
            <!-- Content via JS -->
        </div>
    </div>
    ...
    <div class="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center opacity-0 pointer-events-none transition-all duration-300"
        id="modal-delete-port-overlay">
        <div
            class="bg-white rounded-[24px] w-full max-w-[400px] shadow-2xl overflow-hidden transform scale-95 transition-all duration-300">
            <div class="px-8 pt-8 pb-6 text-center">
                ...
                <p class="text-[13.5px] text-slate-500 leading-relaxed">Portofolio ini akan dihapus permanen dan tidak dapat
                    dikembalikan.</p>
            </div>
            <div class="flex gap-3 px-8 pb-8">
                <button onclick="window.closePortDeleteModal()"
                    class="flex-1 py-3.5 rounded-xl bg-slate-100 text-slate-600 font-bold text-[13px] hover:bg-slate-200 transition-all">Batal</button>
                <button id="btn-confirm-delete-port"
                    class="flex-1 py-3.5 rounded-xl bg-red-500 text-white font-bold text-[13px] hover:bg-red-600 transition-all shadow-lg shadow-red-200">Ya,
                    Hapus</button>
            </div>
        </div>
    </div>
