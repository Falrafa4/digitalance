...
@section('content')
  <section class="animate-fadeUp">
    ...
    @if($reviews->isEmpty())
      <x-ui.empty-state icon="ri-star-line" title="Belum ada review"
        description="Review akan muncul di sini setelah Anda memberikan rating pada order yang selesai."
        actionUrl="{{ route('client.orders.index') }}" actionLabel="Lihat Orders" />
    @else
          ...
          <div class="bg-white border border-slate-200 rounded-[18px] p-5 hover:shadow-md transition-all">
            <div class="flex items-start justify-between gap-4 mb-4 pb-4 border-b border-slate-100">
              ...
              <button type="button" data-review-delete data-order-id="{{ $review->order_id }}"
                class="px-3 py-1.5 rounded-lg bg-red-50 text-red-600 text-[11px] font-bold hover:bg-red-100 transition-all">
            ...
            <div class="flex gap-1 mb-3">
              @for($i = 1; $i <= 5; $i++)
                @php $starColor = $i <= $review->rating ? 'text-amber-400' : 'text-slate-200'; @endphp
                <i class="ri-star-fill {{ $starColor }} text-[18px]"></i>
              @endfor
            </div>
  ...
  <div
    class="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center opacity-0 pointer-events-none transition-all duration-300"
    id="modal-delete-review-client">
    <div
      ...
      <div class="px-8 pt-8 pb-6 text-center">
        ...
        <p class="text-[13.5px] text-slate-500 leading-relaxed">Review ini akan dihapus permanen dan tidak dapat
          dikembalikan.</p>
      </div>
      <div class="flex gap-3 px-8 pb-8">
        <button onclick="window.closeDeleteReviewClient()"
        ...
        <button id="btn-confirm-delete-review-client"
  ...
  <script>
    ...
    document.querySelectorAll('[data-review-delete]').forEach((button) => {
      button.addEventListener('click', function () {
        window.openDeleteReview(this.dataset.orderId);
      });
    });
  </script>
@endsection