...
<section class="animate-fadeUp">
  <div class="flex flex-col lg:flex-row gap-6">
    <div class="flex-1 min-w-0 space-y-6">
      ...
      <div x-data="{ showNego: false, showReject: false, isSubmitting: false }" class="bg-white border border-slate-200 rounded-[18px] p-6">
        ...
        <div x-show="showNego" x-init="$watch('showNego', value => { if(value) { $nextTick(() => window.DigitalanceUtils.focusTrap($el)) } })" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <div class="relative w-full max-w-lg bg-white rounded-[24px] shadow-xl p-6 sm:p-8">
            <button @click="showNego = false" class="absolute top-4 right-4 w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500" aria-label="Tutup modal">
              <i class="ri-close-line"></i>
            </button>
            <h2 class="font-display text-xl font-bold text-slate-900 mb-1">Ajukan Negosiasi</h2>
            <form action="{{ route('client.orders.nego', $order->id) }}" method="POST" class="space-y-4" @submit="isSubmitting = true">
              @csrf
              <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Alasan <span class="text-red-500">*</span></label>
                <textarea name="reason" rows="2" required class="w-full px-4 py-3 rounded-[12px] border border-slate-200 focus:border-amber-400 outline-none text-sm"></textarea>
              </div>
              <div>
                ...
                <div class="relative">
                  ...
                  <input type="number" name="new_price" required min="1000" class="w-full pl-10 pr-4 py-3 rounded-[12px] border border-slate-200 focus:border-amber-400 outline-none text-sm font-semibold">
                </div>
              </div>
              <div class="flex gap-3 pt-2">
                ...
                <button type="submit" :disabled="isSubmitting" class="flex-1 py-3 rounded-[12px] bg-amber-500 text-white font-bold text-sm hover:bg-amber-600 disabled:opacity-50">
        ...
        {{-- Modal Tolak --}}
        <div x-show="showReject" x-init="$watch('showReject', value => { if(value) { $nextTick(() => window.DigitalanceUtils.focusTrap($el)) } })" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <div class="relative w-full max-w-md bg-white rounded-[20px] shadow-xl p-6">
            ...
            <form action="{{ route('client.orders.reject', $order->id) }}" method="POST" class="space-y-4" @submit="isSubmitting = true">
              @csrf
              ...
              <div class="flex gap-3">
                ...
                <button type="submit" :disabled="isSubmitting" class="flex-1 py-2.5 rounded-[12px] bg-red-600 text-white font-bold text-sm hover:bg-red-700 disabled:opacity-50">
      ...
      <div x-data="{ showRevision: false, isSubmitting: false }" class="bg-white border border-slate-200 rounded-[18px] p-6">
        <div class="flex items-center justify-between gap-3 mb-4">
          ...
          <button @click="showRevision = true" class="px-4 py-2.5 rounded-[12px] bg-amber-50 border border-amber-200 text-amber-700 font-bold text-[13px] hover:bg-amber-100 transition-all" aria-label="Ajukan revisi">
            <i class="ri-add-line mr-1"></i> Ajukan Revisi
          </button>
        </div>

        <div x-show="showRevision" x-init="$watch('showRevision', value => { if(value) { $nextTick(() => window.DigitalanceUtils.focusTrap($el)) } })" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          <div @click="showRevision = false" class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm"></div>
          <div class="relative w-full max-w-lg bg-white rounded-[24px] shadow-xl p-6 sm:p-8">
            <button @click="showRevision = false" class="absolute top-4 right-4 w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500" aria-label="Tutup modal"><i class="ri-close-line"></i></button>
            <h2 class="font-display text-xl font-bold text-slate-900 mb-1">Ajukan Revisi</h2>
            <form action="{{ route('client.orders.revision', $order->id) }}" method="POST" class="space-y-4" @submit="isSubmitting = true">
              @csrf
              <textarea name="reason" rows="3" required class="w-full px-4 py-3 rounded-[12px] border border-slate-200 text-sm" placeholder="Bagian yang ingin direvisi..."></textarea>
              <div class="flex gap-3">
        ...
        @else
          <form method="POST" action="{{ route('client.reviews.store') }}" class="mt-4 space-y-4">
            ...
            <div class="flex gap-4 items-center">
              ...
              <input name="comment" class="flex-1 px-4 py-2.5 rounded-[12px] bg-slate-50 border border-slate-200" placeholder="Komentar singkat...">
            </div>
            <button class="px-6 py-2.5 rounded-[12px] bg-slate-900 text-white font-bold text-[13px] hover:bg-black transition-all">Kirim Review</button>
          </form>
        @endif
      </div>
    </div>
