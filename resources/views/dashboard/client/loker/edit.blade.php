                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <select name="category_id"
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" @if($loker->category_id == $cat->id) selected @endif>{{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                <div class="flex flex-col gap-1.5">
                    <label class="text-[11px] font-bold text-slate-500 uppercase tracking-[.1em]">Budget Minimal
                        (Rp)</label>
                    <input type="text" name="budget_min" value="{{ old('budget_min', $loker->budget_min) }}"
                        data-rupiah-input inputmode="numeric"
                        class="py-[10px] px-[13px] bg-slate-50 border-[1.5px] border-slate-200 rounded-[11px] text-[13.5px] outline-none transition-all duration-200 focus:border-[#0f766e] focus:bg-white" />
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-[11px] font-bold text-slate-500 uppercase tracking-[.1em]">Budget Maksimal
                        (Rp)</label>
                    <input type="text" name="budget_max" value="{{ old('budget_max', $loker->budget_max) }}"
                        data-rupiah-input inputmode="numeric"
                        class="py-[10px] px-[13px] bg-slate-50 border-[1.5px] border-slate-200 rounded-[11px] text-[13.5px] outline-none transition-all duration-200 focus:border-[#0f766e] focus:bg-white" />
                </div>
            </div>
