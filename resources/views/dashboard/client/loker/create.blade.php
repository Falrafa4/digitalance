...
@section('content')
    <div class="content-scroll flex-1 px-8 py-7 overflow-y-auto max-w-3xl relative overflow-hidden">
        ...
        <div class="mb-8 animate-fadeUp relative z-10">
            ...
            <p class="text-slate-500 text-[0.95rem] mt-1">Jelaskan project yang kamu butuhkan agar freelancer bisa melamar.
            </p>
        </div>
        ...
        <form action="{{ route('client.loker.store') }}" method="POST"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
                <div class="flex flex-col gap-1.5">
                    <label class="text-[11px] font-bold text-slate-500 uppercase tracking-[.1em]">Budget Minimal
                        (Rp)</label>
                    <input type="text" name="budget_min" value="{{ old('budget_min') }}" data-rupiah-input
                        inputmode="numeric" placeholder="Contoh: 50.000"
                        class="py-[10px] px-[13px] bg-slate-50 border-[1.5px] border-slate-200 rounded-[11px] text-[13.5px] outline-none transition-all duration-200 focus:border-[#0f766e] focus:bg-white focus:shadow-[0_0_0_3px_rgba(15,118,110,0.08)]" />
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-[11px] font-bold text-slate-500 uppercase tracking-[.1em]">Budget Maksimal
                        (Rp)</label>
                    <input type="text" name="budget_max" value="{{ old('budget_max') }}" data-rupiah-input
                        inputmode="numeric" placeholder="Contoh: 500.000"
                        class="py-[10px] px-[13px] bg-slate-50 border-[1.5px] border-slate-200 rounded-[11px] text-[13.5px] outline-none transition-all duration-200 focus:border-[#0f766e] focus:bg-white focus:shadow-[0_0_0_3px_rgba(15,118,110,0.08)]" />
                </div>
            </div>
