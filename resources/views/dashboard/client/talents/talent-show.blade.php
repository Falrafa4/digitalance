      ...
      <div class="xl:col-span-2">
        <div class="bg-white border border-slate-200 rounded-[18px] p-6">
          <div class="flex items-end justify-between gap-4">
            ...
            <a href="{{ route('client.services.index') }}" class="px-4 py-2.5 rounded-[12px] bg-white border border-slate-200 text-slate-700 font-bold text-[12.5px]
          ...
          @if(empty($services) || count($services) === 0)
            <x-ui.empty-state icon="ri-tools-line" title="Belum ada layanan"
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4" data-pager-list>
                @foreach($services as $s)
                  <a data-pager-item href="{{ route('client.services.show', $s->id) }}"
                    ...
                    <p class="text-slate-500 text-[13px] mt-1">Kategori: <span
                        class="font-bold">{{ $s->service_category->name ?? '-' }}</span></p>
                    <div class="mt-3">
      ...
      <aside>
        <div class="bg-white border border-slate-200 rounded-[18px] p-6">
          ...
          <p class="text-slate-500 text-[13.5px] mt-1">
            Email: <span
              class="font-extrabold text-slate-900">{{ optional($freelancer->skomda_student)->email ?? '-' }}</span>
          </p>
