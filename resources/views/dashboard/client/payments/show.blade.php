...
@section('content')
  <section class="animate-fadeUp">
    <div class="mb-6">
      ...
      <h1 class="font-display text-[1.85rem] font-extrabold text-slate-900 mt-2">Transaksi #{{ $transaction->id }}</h1>
    ...
    <div class="bg-white border border-slate-200 rounded-[18px] p-6">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p class="text-slate-500 text-[13.5px] mt-2">
            Dibuat: <span
              class="font-extrabold text-slate-900">{{ optional($transaction->created_at)->format('d M Y H:i') }}</span>
          </p>
        </div>
      ...
      <div class="mt-5 pt-5 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
        ...
        @endif
        <a href="{{ route('client.payments.index') }}" class="px-5 py-3 rounded-[12px] bg-white border border-slate-200 text-slate-700 font-bold text-[13px]
