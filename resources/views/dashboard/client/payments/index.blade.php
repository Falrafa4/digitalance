...
@section('content')
  <section class="animate-fadeUp">
    <div class="mb-6">
      <h1 class="font-display text-[1.85rem] font-extrabold text-slate-900">Pembayaran</h1>
      <p class="text-slate-500 mt-1">Daftar transaksi kamu.</p>
    </div>

    @if(empty($transactions) || count($transactions) === 0)
      <x-ui.empty-state icon="ri-bank-card-line" title="Belum ada transaksi"
        description="Transaksi akan muncul setelah ada pembayaran." actionUrl="{{ route('client.orders.index') }}"
          ...
          @foreach($transactions as $t)
            <div data-pager-item class="bg-white border border-slate-200 rounded-[18px] p-5 hover:shadow-lg transition-all">
              <div class="flex items-start justify-between gap-4">
                <div>
                  <p class="font-extrabold text-slate-900">Transaksi #{{ $t->id }}</p>
                  <p class="text-slate-500 text-[13px] mt-1">Pesanan #{{ $t->order_id ?? '-' }}</p>
              ...
              <div class="flex gap-2 mt-4">
                @if(!empty($t->order_id))
                  ...
                  <a href="{{ route('client.orders.show', $t->order_id) }}" class="flex-1 px-4 py-2.5 rounded-[12px] bg-white border border-slate-200 text-slate-700 font-bold text-[12.5px]
