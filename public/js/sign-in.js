document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.serviceCategories && Array.isArray(window.serviceCategories)) {
        if (typeof window.serviceCategories[0] === 'object') {
            availableCategories = window.serviceCategories.map(cat => cat.name || cat.title || '');
        } else {
    ...
    if (tagsContainer && skillInput && skillSuggestions) {
      function renderTags() {
        ...
        skills.forEach(skill => {
          ...
          // MENGGUNAKAN INLINE SVG AGAR 'X' PASTI MUNCUL TANPA FONT-AWESOME
      ...
      // Deteksi klik tombol X untuk menghapus skill
