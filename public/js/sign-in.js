document.addEventListener('DOMContentLoaded', () => {
  ...
  function getStudentByNis(nis) {
    return getStudents().find(s => String(s.nis) === String(nis));
  }

  // isi email siswa berdasarkan NIS terpilih
  function updateStudentEmailUI() {
    ...
    const nis = (studentSelect.value || '').trim();
    const s = getStudentByNis(nis);
    studentEmailInput.value = s ? (s.email || '') : '';
  }
  ...
  function checkFormValidity() {
    ...
    if (currentRole === 'freelancer') {
      const nis = studentSelect ? studentSelect.value.trim() : '';
      const ok = !!getStudentByNis(nis);
  ...
  function updateRole(role) {
    ...
    if (freelancerFields) freelancerFields.classList.toggle('hidden', !isFreelancer);
    ...
    if (isFreelancer) updateStudentEmailUI();
  ...
  if (studentSelect && nisInput) {
    // kalau studentSelect = <select>, pakai 'change'. kalau nanti diganti <input>, ini masih aman.
    const evtName = (studentSelect.tagName && studentSelect.tagName.toLowerCase() === 'input') ? 'input' : 'change';

    studentSelect.addEventListener(evtName, () => {
      nisInput.value = (studentSelect.value || '').trim();
      updateStudentEmailUI();
      checkFormValidity();
    });

    // init old value (berguna jika ada error validation dari Laravel)
    if (nisInput.value && !studentSelect.value) {
      studentSelect.value = nisInput.value;
    }

    // fill email on load
    nisInput.value = (studentSelect.value || nisInput.value || '').trim();
    updateStudentEmailUI();
    checkFormValidity();
  }

  // SKILLS TAG & AUTOCOMPLETE
  if (tagsContainer && skillInput && skillSuggestions && tagLimitMsg && hiddenSkills) {
    ...
    function showSuggestions(val) {
      if (!val || skills.length >= MAX_SKILLS || availableCategories.length === 0) {
        hideSuggestions();
        return;
      }

      const matches = availableCategories.filter(cat =>
        cat.toLowerCase().includes(val.toLowerCase()) && !skills.includes(cat)
      );
      ...
      skillSuggestions.innerHTML = matches.map(match =>
    ...
    document.addEventListener('click', (e) => {
      if (!tagsContainer.contains(e.target) && !skillSuggestions.contains(e.target)) {
        hideSuggestions();
      }
    });
  }
